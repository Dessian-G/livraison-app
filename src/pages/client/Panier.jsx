...
export default function Panier() {
  ...
  if (articles.length === 0) return (
    <div className="text-center py-20">
      ...
      <Link to="/" className="inline-block bg-bleu text-white px-6 py-2.5 rounded-xl font-medium hover:bg-bleu-dark transition">
        Voir les produits
      </Link>
    </div>
  )
  ...
  return (
    <div>
        ...
        {articles.map(a => (
          <div key={a.produitId} className="bg-white rounded-xl shadow-sm p-4 flex items-center gap-4">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-bleu font-bold text-sm mt-0.5">{(a.prix * a.quantite).toLocaleString()} FCFA</p>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => modifierQuantite(a.produitId, a.quantite + 1)}
                className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-100 hover:bg-bleu/10 hover:text-bleu transition"
              >
                <Plus size={14} />
              </button>
            </div>
      ...
      <div className="bg-white rounded-xl shadow-sm p-4 mb-4 flex items-center justify-between">
        ...
        <span className="text-xl font-bold text-bleu">{total.toLocaleString()} FCFA</span>
      </div>
